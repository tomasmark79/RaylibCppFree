...
std::unique_ptr<dotname::RaylibWrapper> uniqueLib;
...
int handlesArguments (int argc, const char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::RaylibWrapper> ();
      uniqueLib = std::make_unique<dotname::RaylibWrapper> (AppContext::assetsPath);
    } else {
